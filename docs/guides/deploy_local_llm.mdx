...
Tessi ai. supports deploying models locally using Ollama or Xinference. If you have locally deployed models to leverage or wish to enable GPU or CUDA for inference acceleration, you can bind Ollama or Xinference into Tessi ai. and use either of them as a local "server" for interacting with your local models.

Tessi ai. seamlessly integrates with Ollama and Xinference, without the need for further environment configurations. You can use them to deploy two types of local models in Tessi ai.: chat models and embedding models.

:::tip NOTE
This user guide does not intend to cover much of the installation or configuration details of Ollama or Xinference; its focus is on configurations inside Tessi ai.. For the most current information, you may need to check out the official site of Ollama or Xinference.
:::
...
In Tessi ai., click on your logo on the top right of the page **>** **Model Providers** and add Ollama to Tessi ai.:
...
3. Ensure that the base URL you enter is accessible to Tessi ai..
4. OPTIONAL: Switch on the toggle under **Does it support Vision?** if your model includes an image-to-text model.

:::caution NOTE
- If your Ollama and Tessi ai. run on the same machine, use `http://localhost:11434` as base URL.
- If your Ollama and Tessi ai. run on the same machine and Ollama is in Docker, use `http://host.docker.internal:11434` as base URL.
- If your Ollama runs on a different machine from Tessi ai., use `http://<IP_OF_OLLAMA_MACHINE>:11434` as base URL.
:::
...
In Tessi ai., click on your logo on the top right of the page **>** **Model Providers** and add Xinference to Tessi ai.:
...
To enable IPEX-LLM accelerated Ollama in Tessi ai., you must also complete the configurations in Tessi ai.. The steps are identical to those outlined in the *Deploy a local model using Ollama* section:
