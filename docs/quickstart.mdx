...
Tessi ai. is an open-source RAG (Retrieval-Augmented Generation) engine based on deep document understanding. When integrated with LLMs, it is capable of providing truthful question-answering capabilities, backed by well-founded citations from various complex formatted data.

This quick start guide describes a general process from:

- Starting up a local Tessi ai. server,
...
This section provides instructions on setting up the Tessi ai. server on Linux. If you are on a different operating system, no worries. Most steps are alike.
...
<details>
  <summary>1. Ensure <code>vm.max_map_count</code> &ge; 262144:</summary>
   ...
   Tessi ai. v0.10.0 uses Elasticsearch for multiple recall. Setting the value of `vm.max_map_count` correctly is crucial to the proper functioning of the Elasticsearch component.
...
Tessi ai. is a RAG engine, and it needs to work with an LLM to offer grounded, hallucination-free question-answering capabilities. For now, Tessi ai. supports the following LLMs, and the list is expanding:
...
:::note 
Tessi ai. also supports deploying LLMs locally using Ollama, Xinference, or LocalAI, but this part is not covered in this quick start guide.
:::
...
1. Click on your logo on the top right of the page **>** **Model Providers**:
   ...
   > Each Tessi ai. account is able to use **text-embedding-v2** for free, an embedding model of Tongyi-Qianwen. This is why you can see Tongyi-Qianwen in the **Added models** list. And you may need to update your Tongyi-Qianwen API key at a later point.
...
You are allowed to upload files to a knowledge base in Tessi ai. and parse them into datasets. A knowledge base is virtually a collection of datasets. Question answering in Tessi ai. can be based on a particular knowledge base or multiple knowledge bases. File formats that Tessi ai. supports include documents (PDF, DOC, DOCX, TXT, MD), tables (CSV, XLSX, XLS), pictures (JPEG, JPG, PNG, TIF, GIF), and slides (PPT, PPTX).
...
3. Tessi ai. offers multiple chunk templates that cater to different document layouts and file formats. Select the embedding model and chunk method (template) for your knowledge base.
...
Conversations in Tessi ai. are based on a particular knowledge base or multiple knowledge bases. Once you have created your knowledge base and finished file parsing, you can go ahead and start an AI conversation.
...
1. Click the **Chat** tab in the middle top of the mage **>** **Create an assistant** to show the **Chat Configuration** dialogue *of your next dialogue*.
   > Tessi ai. offer the flexibility of choosing a different chat model for each dialogue, while allowing you to set the default models in **System Model Settings**.
...
5. Tessi ai. also offers conversation APIs. Hover over your dialogue **>** **Chat Bot API** to integrate Tessi ai.'s chat capabilities into your applications:
